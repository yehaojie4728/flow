#!/usr/bin/python3
"""
eBPF tracer for AscendCL memcpy events — FlowGap edition.

Adapted from /root/FlowGap-work/reference_repo/src/trace_memcpy_numa.py
FIX: adds NNAE 7.0.0 library path for MindSpore 2.2.14 compatibility.

Changed lines:
  - resolve_ascendcl_lib(): added NNAE 7.0.0 search paths and env var ASCEND_NNAE_LIB

All other logic is identical to the reference repo version.
"""
...
parser.add_argument("--output", type=str, default="aclrtMemcpy_numa_trace.log", help="Output file path")
...
def resolve_ascendcl_lib():
    """Resolve libascendcl.so — FlowGap edition with NNAE paths."""
    candidates = []
    ...
    env_candidates = [
        ...
        os.environ.get("ASCEND_NNAE_LIB"),        # FlowGap: explicit NNAE override
    ]
    for env_path in env_candidates:
        _append_candidate(candidates, env_path)

    # ---- NNAE paths (FlowGap: searched BEFORE TOOLKIT_HOME to avoid CANN override) ----
    for path in (
        "/usr/local/Ascend/nnae/7.0.0/aarch64-linux/lib64/libascendcl.so",
        "/usr/local/Ascend/nnae/7.0.0/lib64/libascendcl.so",
        "/usr/local/Ascend/nnae/latest/aarch64-linux/lib64/libascendcl.so",
        "/usr/local/Ascend/nnae/latest/lib64/libascendcl.so",
        "/usr/local/Ascend/nnae/aarch64-linux/lib64/libascendcl.so",
    ...
    # ---- glob search (exclude devlib copies) ----
    for pattern in (
    ...
    raise FileNotFoundError(
        "未找到 libascendcl.so，请设置 ASCEND_CL_LIB / ASCEND_NNAE_LIB / ASCEND_TOOLKIT_HOME。"
    )
...
# -----------------------------------------------------------------------------
# 2. BPF program (identical to reference repo)
# -----------------------------------------------------------------------------
bpf_text = """
...
int probe_return(struct pt_regs *ctx) {
    ...
    if (val != 0) {
        ...
        data.current_device = (dev_ptr) ? *dev_ptr : -1;
...
def print_event(cpu, data, size):
    ...
    dst_str = "?"
    if event.kind == 0:
        src_str = host_desc; dst_str = host_desc
    elif event.kind == 1:
        src_str = host_desc; dst_str = dev_desc
    elif event.kind == 2:
        src_str = dev_desc; dst_str = host_desc
    elif event.kind == 3:
        src_str = dev_desc; dst_str = dev_desc
    else:
        src_str = "Unknown"; dst_str = "Unknown"
    ...
    f.write(log_line + "\n")
    f.flush()
    merge_or_flush_window(src_str, dst_str, start_ns, end_ns, wall_start_ns, wall_end_ns, float(event.count))
